...
<script>
import { addEmailTemplate, delEmailTemplate, getEmailTemplate, listEmailTemplate, updateEmailTemplate } from '@/api/admin/tb-email-template'
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: { subject: [{ required: true, message: '不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listEmailTemplate(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
        this.emailTemplateList = response.data.list
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除编号为"' + Ids + '"的数据项?', '警告', {
        ...
        return delEmailTemplate({ 'ids': Ids })
      }).then((response) => {
        if (response.code === 200) {
          this.msgSuccess(response.msg)
          this.open = false
